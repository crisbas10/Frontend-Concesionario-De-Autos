...
export class ClienteListComponent implements AfterViewInit {
  ...
  readonly displayedColumns = ['id', 'nombre', 'telefono', 'correo', 'acciones'];
  ...
  editar(row: ClienteRead): void { this.openDialog({ mode: 'edit', row }); }

  private openDialog(data: ClienteDialogData): void {
    this.dialog.open(ClienteDialogComponent, { width: '480px', data }).afterClosed().pipe(filter(Boolean)).subscribe(() => this.reload());
  }

  eliminar(row: ClienteRead): void {
    if (!confirm(`¿Eliminar cliente ${row.nombre}?`)) return;
    this.svc.delete(row.id).subscribe({
      next: () => { this.snack.open('Cliente eliminado', 'OK', { duration: 3000 }); this.reload(); },
