...
export class UsuarioListComponent implements AfterViewInit {
  ...
  readonly displayedColumns = ['id', 'nombre_usuario', 'correo', 'rol', 'acciones'];
  ...
  reload(): void {
    ...
    this.svc.list().subscribe({
      ...
      error: (err: HttpErrorResponse) => { this.loading = false; this.snack.open(this.msg(err), 'Cerrar', { duration: 6000 }); },
    });
  }
  ...
  private openDialog(data: UsuarioDialogData): void {
    this.dialog.open(UsuarioDialogComponent, { width: '480px', data }).afterClosed().pipe(filter(Boolean)).subscribe(() => this.reload());
  }
